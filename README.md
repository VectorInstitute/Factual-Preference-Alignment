...
# 🧠 AIXpert Preference Alignment – DPO Training
...
It is designed for **Vector Institute’s AIXpert project**, supporting reproducible training, evaluation, and checkpointing workflows.
...
## 🧠 Inference Generation – Repeated Sampling
...
the next step is to **generate evaluation prompts and structured outputs** across different templates — specifically `guide` and `guide_reverse`.  

This script automates the process of converting question–answer pairs into evaluation-style prompts and uses a pretrained LLM (e.g., `Qwen2-7B-Instruct`) to produce reasoned preference justifications.  
...
It parses the model’s reasoning outputs, identifies preference-aligned samples, and builds positive–negative pairs used to train the reward-aligned model.
...
this script provides a **quick inspection and summary** of the saved dataset — verifying that the train/validation splits and pair-type distributions are correct.
...
It measures how well each model predicts the **preferred (chosen)** answers from the DPO dataset — effectively computing **alignment accuracy**.
